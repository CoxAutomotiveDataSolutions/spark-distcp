...
object PathUtils {
  ...
  /**
    ...
    * This function determines how a source file path is mapped to the destination.
    * The behaviour is different depending on if update or overwrite is used.
    * This follows the behaviour of Hadoop DistCP.
    * See the Hadoop DistCP documentation for more explanation of this behaviour.
    *
    ...
    */
  def sourceURIToDestinationURI(file: URI, sourceURI: URI, destinationURI: URI, updateOverwritePathBehaviour: Boolean): URI = {
    val sourceFolderURI: URI = {
      if (updateOverwritePathBehaviour) sourceURI
      else Option(new Path(sourceURI).getParent).map(_.toUri).getOrElse(sourceURI)
    }
