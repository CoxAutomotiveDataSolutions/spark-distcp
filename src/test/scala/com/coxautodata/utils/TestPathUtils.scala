...
class TestPathUtils extends AnyFunSpec with Matchers {
  ...
  describe("pathToQualifiedPath") {
    ...
    it("qualify a path with the default fs") {
      ...
      pathToQualifiedPath(new Configuration(), path).toString should be(
        "file:/test"
      )
    }
  ...
  describe("sourcePathToDestinationPath") {
    ...
    it("non-update/non-overwrite behaviour") {
      ...
      sourceURIToDestinationURI(
        ...
        updateOverwritePathBehaviour = false
      ) should be(new Path("hdfs://nn2:8020/target/first/1").toUri)
    ...
    it("update/overwrite behaviour") {
      ...
      sourceURIToDestinationURI(
        ...
        new Path("hdfs://nn1:8020/source/first").toUri,
        new Path("hdfs://nn2:8020/target").toUri,
        updateOverwritePathBehaviour = true
      ) should be(new Path("hdfs://nn2:8020/target/1").toUri)
  ...
  describe("uriIsChild") {
    ...
    it("check whether path is a child") {
      ...
      uriIsChild(
        new URI("file:/test/folder"),
        new URI("file:/test/folder/child")
      ) should be(true)
      uriIsChild(
        new URI("file:/test/folder"),
        new URI("file:/test/folder")
      ) should be(true)
      uriIsChild(new URI("file:/test/folder"), new URI("file:/test")) should be(
        false
      )
      uriIsChild(
        new URI("file:/test/folder"),
        new URI("hdfs:/test/folder/child")
      ) should be(false)
      uriIsChild(
        new URI("file:/test/folder"),
        new URI("hdfs:/test/folder.file")
      ) should be(false)
