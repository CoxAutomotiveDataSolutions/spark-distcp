...
class TestAccumulators extends AnyFunSpec with Matchers {
  ...
  it("test all accumulator conditions") {
    ...
    val testCases: Seq[DistCPResult] = List(
      ...
      DeleteResult(
        null,
        DeleteActionResult.Failed(new RuntimeException("test"))
      ),
      ...
      DirectoryCopyResult(
        null,
        null,
        CopyActionResult.Failed(new RuntimeException("test"))
      ),
      ...
      FileCopyResult(
        ...
        null,
        50000,
        CopyActionResult.Failed(new FileNotFoundException("test"))
      )
    )
    ...
    acc.getOutputText should be("""--Raw data--
